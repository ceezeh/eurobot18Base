...
astar::RealTrajectoryList astar::AStar::convertPathToRealTrajectory(
	...
	for (int i = 0; i < num; i++) {
		if (i == 0) { // Goal
			trajectory.push_front(stop);
		} else if (i == num - 1) {
			trajectory.push_front(start);
		} else {
...
Pose_t astar::AStar::getSubGoal(Pose_t currentLocation) {
	if (!isInitialised || trajectory.empty()) {
		...
		while (convertToPoint(currentLocation)==convertToPoint(trajectory.front())
				&& !trajectory.empty()) {
			cout <<"[Removing subgoal], x"<<trajectory.front().x<<", y:"<<trajectory.front().y<<endl;
