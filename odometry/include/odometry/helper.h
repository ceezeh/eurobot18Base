...
struct Pose_t {
	...
	bool closeTo(Pose_t p) { // TODO: Be mindful of the threshold. May affect odometry accuracy.
		if (closePointTo(p) && (abs(angDiff(this->th, p.th)) < 0.5)) {
			return true;
		} else {
			return false;
		}
	...
	bool closePointTo(Pose_t p) { // TODO: Be mindful of the threshold. May affect odometry accuracy.
		if (sqrt(pow(this->x - p.x, 2) + pow(this->y - p.y, 2)) < .1) {
			return true;
		} else {
			return false;
		}
	}
	bool closerPointTo(Pose_t p) { // TODO: Be mindful of the threshold. May affect odometry accuracy.
		if (sqrt(pow(this->x - p.x, 2) + pow(this->y - p.y, 2)) < .03) {
			return true;
		} else {
			return false;
		}
