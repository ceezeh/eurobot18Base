...
void ParticleFilter::prediction(double delta_t, double std_pos[],
	...
	for (int i = 0; i < num_particles; ++i) {
		...
		// Normal distribution for x, y and theta
		normal_distribution<double> dist_v(velocity, std_x);
		normal_distribution<double> dist_w(yaw_rate, std_theta);

		double v = dist_v(gen);
		double w = dist_w(gen);
		double new_x = p->x + v * cos(p->theta + w);
		double new_y = p->y + v * sin(p->theta + w);
		double new_theta = p->theta + (w);
...
geometry_msgs::Pose ParticleFilter::getPosition() {
	...
	if (isinf(x) || isnan(x) || isinf(y) || isnan(y) || isnan(theta)
			|| isnan(theta)) {
		p.orientation.w = 1000000; // Hack to represent null value.
	} else {

		p.position.x = x;
		p.position.y = y;
		cout << "\rx: " << x << ", y: " << y << ", th: " << theta;
